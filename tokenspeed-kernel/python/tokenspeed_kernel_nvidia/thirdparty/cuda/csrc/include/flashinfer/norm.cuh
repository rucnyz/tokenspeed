...
template <uint32_t VEC_SIZE, typename T>
__global__ void RMSNormFusedParallelKernel(
    T* __restrict__ input1, T* __restrict__ input2,
    T* __restrict__ weight1, T* __restrict__ weight2,
    T* __restrict__ output1, T* __restrict__ output2,
    const uint32_t d1, const uint32_t d2,
    const uint32_t stride_input1, const uint32_t stride_input2,
    const uint32_t stride_output1, const uint32_t stride_output2,
    const uint32_t num_threads_1, const uint32_t num_threads_2,
    float eps) {
  const uint32_t bx = blockIdx.x;
  const uint32_t tx = threadIdx.x, ty = threadIdx.y;
  constexpr uint32_t warp_size = 32;
  ...
  if (thread_id < num_threads_1) {
    ...
    for (uint32_t i = 0; i < rounds_1; i++) {
      vec_t<T, VEC_SIZE> input_vec;
      input_vec.fill(0.f);
      if ((i * num_threads_1 + thread_id) * VEC_SIZE < d1) {
        input_vec.load(input1 + bx * stride_input1 + i * num_threads_1 * VEC_SIZE + thread_id * VEC_SIZE);
      }
#pragma unroll
      for (uint32_t j = 0; j < VEC_SIZE; j++) {
        sum_sq += float(input_vec[j]) * float(input_vec[j]);
    ...
    for (uint32_t i = 0; i < rounds_2; i++) {
      vec_t<T, VEC_SIZE> input_vec;
      input_vec.fill(0.f);
      if ((i * num_threads_2 + local_thread_id) * VEC_SIZE < d2) {
        input_vec.load(input2 + bx * stride_input2 + i * num_threads_2 * VEC_SIZE + local_thread_id * VEC_SIZE);
      }
#pragma unroll
      for (uint32_t j = 0; j < VEC_SIZE; j++) {
        sum_sq += float(input_vec[j]) * float(input_vec[j]);
...
#pragma unroll
  ...
  if (tx == 0) {
    if (ty < num_warps_1) {
      smem_warp_sum_sq_1[ty] = sum_sq;
    } else if (ty < num_warps_1 + num_warps_2) {
      smem_warp_sum_sq_2[ty - num_warps_1] = sum_sq;
    }
  }
  __syncthreads();

  // ========== Phase 3: Cross-warp reduce ==========
  if (ty == 0) {
    float sum_sq_1 = 0.f;
    float sum_sq_2 = 0.f;
...
#pragma unroll
    for (uint32_t offset = warp_size / 2; offset > 0; offset /= 2) {
      sum_sq_2 += math::shfl_xor_sync(sum_sq_2, offset);
    }

    if (tx == 0) {
      smem_warp_sum_sq_1[0] = math::rsqrt(sum_sq_1 / float(d1) + eps);
      smem_warp_sum_sq_2[0] = math::rsqrt(sum_sq_2 / float(d2) + eps);
    }
  }
  __syncthreads();

  float rms_rcp_1 = smem_warp_sum_sq_1[0];
  float rms_rcp_2 = smem_warp_sum_sq_2[0];
  ...
  if (thread_id < num_threads_1) {
    ...
    for (uint32_t i = 0; i < rounds_1; i++) {
      ...
      if ((i * num_threads_1 + thread_id) * VEC_SIZE < d1) {
        input_vec.load(input1 + bx * stride_input1 + i * num_threads_1 * VEC_SIZE + thread_id * VEC_SIZE);
        weight_vec.load(weight1 + i * num_threads_1 * VEC_SIZE + thread_id * VEC_SIZE);
      }
#pragma unroll
      for (uint32_t j = 0; j < VEC_SIZE; j++) {
        output_vec[j] = float(input_vec[j]) * rms_rcp_1 * float(weight_vec[j]);
      }
      if ((i * num_threads_1 + thread_id) * VEC_SIZE < d1) {
        output_vec.store(output1 + bx * stride_output1 + i * num_threads_1 * VEC_SIZE +
                         thread_id * VEC_SIZE);
      }
    }
  } else if (thread_id < num_threads_1 + num_threads_2) {
    ...
    for (uint32_t i = 0; i < rounds_2; i++) {
      ...
      if ((i * num_threads_2 + local_thread_id) * VEC_SIZE < d2) {
        input_vec.load(input2 + bx * stride_input2 + i * num_threads_2 * VEC_SIZE + local_thread_id * VEC_SIZE);
        weight_vec.load(weight2 + i * num_threads_2 * VEC_SIZE + local_thread_id * VEC_SIZE);
      }
#pragma unroll
      for (uint32_t j = 0; j < VEC_SIZE; j++) {
        output_vec[j] = float(input_vec[j]) * rms_rcp_2 * float(weight_vec[j]);
      }
      if ((i * num_threads_2 + local_thread_id) * VEC_SIZE < d2) {
        output_vec.store(output2 + bx * stride_output2 + i * num_threads_2 * VEC_SIZE +
                         local_thread_id * VEC_SIZE);
      }
    }
  }
#if (__CUDACC_VER_MAJOR__ >= 12 && defined(__CUDA_ARCH__) && (__CUDA_ARCH__ >= 900))
...
template <typename T>
cudaError_t RMSNormFusedParallel(
    T* input1, T* weight1, T* output1, uint32_t d1,
    T* input2, T* weight2, T* output2, uint32_t d2,
    uint32_t batch_size, uint32_t stride_input1, uint32_t stride_input2,
    uint32_t stride_output1, uint32_t stride_output2,
    float eps = 1e-5, bool enable_pdl = false, cudaStream_t stream = 0) {
  const uint32_t vec_size1 = std::gcd(16 / sizeof(T), d1);
  ...
  while (num_threads_1 + num_threads_2 > 1024) {
    if (num_threads_1 > num_threads_2) {
      num_threads_1 -= 32;
    } else {
      num_threads_2 -= 32;
    }
  ...
  const uint32_t num_threads = num_threads_1 + num_threads_2;
  ...
  DISPATCH_ALIGNED_VEC_SIZE(vec_size, VEC_SIZE, {
    auto kernel = RMSNormFusedParallelKernel<VEC_SIZE, T>;
    FLASHINFER_CUDA_CALL(
        cudaFuncSetAttribute(kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size));
    FLASHINFER_CUDA_CALL(cudaLaunchKernelEx(&config, kernel, input1, input2, weight1, weight2,
                                            output1, output2, d1, d2, stride_input1, stride_input2,
                                            stride_output1, stride_output2, num_threads_1, num_threads_2, eps));
  });
  return cudaSuccess;
}
