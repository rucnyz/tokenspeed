...
__global__ void VerifyChainGreedyKernel(
    ...
    int bx = threadIdx.x;
    int batch_offset = bx * num_draft_tokens;
    int num_accepted_draft_tokens = 0;
    ...
    for (int i = 1; i < num_draft_tokens; i++){
        ...
        IdType draft_id = candidates[batch_offset + i];
        // draft_id accepted, we can continue
        if (draft_id == target_id){
            // set predict output
            predicts[batch_offset + i - 1] = target_id;
            accept_index[batch_offset + i] = batch_offset + i;
            num_accepted_draft_tokens = i;
        }else{
            break;
        }
    }
...
inline cudaError_t VerifyChainGreedy(
    int32_t* predicts,                 // mutable
    int32_t* accept_index,             // mutable
    int32_t* accept_token_num,         // mutable
    int32_t* candidates,
    int64_t* target_predict,
    uint32_t batch_size,
    uint32_t num_draft_tokens,
...
__global__ void ChainSpeculativeSamplingKernelTargetOnlyFastPath(
  ...
  uint32_t num_speculative_steps = num_draft_tokens - 1;
  ...
  for (uint32_t i = 1; i < num_draft_tokens; i++) {
    ...
    IdType draft_id = candidates[bx * num_draft_tokens + i];
...
template <typename DType, typename IdType>
cudaError_t ChainSpeculativeSamplingTargetOnly(
    IdType* predicts,                   // mutable
    IdType* accept_index,           // mutable
