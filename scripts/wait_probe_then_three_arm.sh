#!/usr/bin/env bash
# After pressure probe completes, run staggered three-arm comparison.
set -euo pipefail
...
PROBE_MARKER="output-dir runs/hima_pressure_probe_20260712/base/rep0"
...
wait_gpu_free() {
  ...
  while true; do
    ...
    if [[ -n "$free_mib" && "$free_mib" -ge "$min_free_mib" ]]; then
      echo "[three_arm] GPU${gpu_id} free=${free_mib}MiB (>= ${min_free_mib}MiB)"
      return 0
    fi
    echo "[three_arm] waiting GPU${gpu_id} free=${free_mib:-?}MiB < ${min_free_mib}MiB"
    sleep 30
  done
}

echo "[three_arm] waiting for pressure probe to exit"
while pgrep -f "$PROBE_MARKER" >/dev/null 2>&1; do
  sleep 60
done
sleep 20
...
run_arm() {
  ...
  echo "[three_arm] $preset on GPU $gpu"
  "${cmd[@]}" > "${OUT}/${sub}/rep0/run.log" 2>&1
  echo "[three_arm] $preset done exit=$?"
}

run_arm base 1 11400 base
