...
def _messages_to_text(messages: list[dict[str, Any]], continue_final_message: bool = False) -> str:
...
def _sampling_params(data: dict[str, Any]) -> dict[str, Any]:
    ...
    for src, dst in mapping.items():
        if src in data and data[src] is not None:
            out[dst] = data[src]
    max_tokens = data.get("max_completion_tokens", data.get("max_tokens", data.get("max_new_tokens")))
    if max_tokens is not None:
        out["max_new_tokens"] = int(max_tokens)
    min_tokens = data.get("min_completion_tokens", data.get("min_tokens", data.get("min_new_tokens")))
...
def _request_to_generate_input(data: dict[str, Any], *, rid: str) -> GenerateReqInput:
    ...
    obj = GenerateReqInput(
        ...
        top_logprobs_num=int(data.get("top_logprobs", data.get("top_logprobs_num", 0)) or 0),
...
def _openai_response(data: dict[str, Any], output: dict[str, Any], *, chat: bool, rid: str) -> dict[str, Any]:
...
async def _stream_generate(data: dict[str, Any], *, chat: bool):
    ...
    async for output in async_llm.generate_request(obj):
        ...
        choice: dict[str, Any] = {"index": int(output.get("index", 0) or 0), "finish_reason": finish}
        if chat:
...
async def completions(request: Request):
    ...
    if data.get("stream", False):
        return StreamingResponse(_stream_generate(data, chat=False), media_type="text/event-stream")
...
async def chat_completions(request: Request):
    data = await request.json()
    if data.get("stream", False):
        return StreamingResponse(_stream_generate(data, chat=True), media_type="text/event-stream")
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Launching TokenSpeed HTTP worker on %s:%s", server_args.host, server_args.port)
